...
class Worker {
  ...
  async produce () {
    await this._queues.addNewMessage('name: ' + this._name + ', time: ' + Date.now())
  }
  ...
  async requeueProcessingMessages () {
    let registeredWorkers = this._discovery.getRegisteredWorkers()

    for (let workerName of registeredWorkers) {
      if (await this._discovery.getWorkerStatus()) {
        continue
      }
