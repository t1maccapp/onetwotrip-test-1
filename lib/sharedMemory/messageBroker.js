...
class MessageBroker {
  ...
  async getErrorsChunk () {
    let errors = await this._asyncLRange(MESSAGES_QUEUE_ERRORS_KEY, -ERRORS_CHUNK_SIZE, -1)

    await this._asyncLTrim(MESSAGES_QUEUE_ERRORS_KEY, 0, -(ERRORS_CHUNK_SIZE + 1))
